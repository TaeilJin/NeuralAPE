...
public class Actor : MonoBehaviour {
	...
	public Matrix4x4[] GetBoneLocalTransformations()
	{
		Matrix4x4[] transformations = new Matrix4x4[Bones.Length];
		for (int i = 0; i < transformations.Length; i++)
		{
			transformations[i] = Bones[i].Transform.GetLocalMatrix();
		}
		return transformations;
	}
	public Matrix4x4[] GetBoneLocalTransformations(params string[] bones)
	{
		Matrix4x4[] transformations = new Matrix4x4[bones.Length];
		for (int i = 0; i < transformations.Length; i++)
		{
			transformations[i] = GetBoneLocalTransformation(bones[i]);
		}
		return transformations;
	}

	public Matrix4x4 GetBoneLocalTransformation(string bone)
	{
		return FindBone(bone).Transform.GetLocalMatrix();
	}
