...
public class EnvMoData : ScriptableObject {
	...
	public void GenerateRootTrajectory(int start, int end)
    {
		...
		for (int n=start, k =0; n <= end; n++,k++)
        {
			...
			MotionWR[k] = new Matrix4x4[data_out.Source.Bones.Length];
			for (int j = 0; j < data_out.Source.Bones.Length; j++)
	...
	public void inspector(playEnvMotion pisd)
    {
